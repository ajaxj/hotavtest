...
const fetchTitanMatches = async (
  page: number = 0,
  pageSize: number = 10
): Promise<TitanFbMatchPage> => {
  // 构造请求 URL，后端页码从 1 开始，所以 page + 1
  const url = `${API_ENDPOINTS.TITAN_FB_MATCHES}?page=${page + 1}&size=${pageSize}`
...
export const useTitanMatches = (pageIndex: number, pageSize: number) => {
  // 任务分页数据状态
  const [data, setData] = useState<TitanFbMatchPage | null>(null)
  // 加载中状态
  ...
  const createTitanMatch = useCallback(
    async (payload: CreateTitanMatchPayload, onSuccess?: () => void) => {
      ...
      try {
        ...
        const res = await apiFetch(API_ENDPOINTS.TITAN_FB_MATCHES, {
          method: "POST",
...
export const useUpdateTitanMatch = () => {
  ...
  const updateTitanMatch = useCallback(
    async (
      ...
      try {
        // 发送 PUT 请求更新指定比赛
        const res = await apiFetch(`${API_ENDPOINTS.TITAN_FB_MATCHES}/${id}`, {
          method: "PUT",
