import type { TitanFbLeague, TitanFbLeaguePage } from "@/types"
...
const fetchTitanLeagues = async (page:number =0 ,pageSize:number=10):Promise<TitanFbLeaguePage> => {
  // 构造请求 URL，后端页码从 1 开始，所以 page + 1
  const url = `${API_ENDPOINTS.TITAN_FB_LEAGUES}?page=${page + 1}&size=${pageSize}`
...
export const useTitanLeagues = (pageIndex: number, pageSize: number) => {
  // 任务分页数据状态
  const [data, setData] = useState<TitanFbLeaguePage | null>(null)
  // 加载中状态
  ...
  const createTitanLeague = useCallback(
    async (payload: CreateTitanLeaguePayload, onSuccess?: () => void) => {
      ...
      try {
        ...
        const res = await apiFetch(API_ENDPOINTS.TITAN_FB_LEAGUES, {
          method: "POST",
...
export const useUpdateTitanLeague = () => {
  ...
  const updateTitanLeague = useCallback(
    async (
      ...
      try {
        // 发送 PUT 请求更新指定联赛
        const res = await apiFetch(`${API_ENDPOINTS.TITAN_FB_LEAGUES}/${id}`, {
          method: "PUT",
